...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    ...
    allCalories := ((float64(runningCaloriesMeanSpeedMultiplier) * averageSpeed * runningCaloriesMeanSpeedShift) * weight / float64(mInKm) * duration * float64(minInH))
    return allCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
   ...
   allCalories := ((walkingCaloriesWeightMultiplier * weight + (math.Pow(averageSpeed, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH) 
   return allCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    avarageSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
    allCalories := (avarageSpeed + swimmingCaloriesMeanSpeedShift) * float64(swimmingCaloriesWeightMultiplier) * weight * duration
    return allCalories
}
